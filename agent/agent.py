...
@dataclasses.dataclass
class GANNetworks:
    """Structure containing the networks for GAN."""

    def __init__(self,
                 observation_network: types.TensorTransformation,
                 generator_network: snt.Module,
                 ...
                 n_gen_samples: int):
        """
        Initializes the GAN networks.
        Args:
            observation_network: Network or function for preprocessing observations.
            generator_network: Generator network module.
            encoder_loc_network: Encoder network for location.
            encoder_scale_network: Encoder network for scale.
            discriminator_network: Discriminator network module.
            z_dim: Dimensionality of the latent variable z.
            n_gen_samples: Number of return distribution samples generated by the generator.
        """
        self.observation_network = tf2_utils.to_sonnet_module(observation_network)
        self.generator_network = generator_network
        self.encoder_loc_network = encoder_loc_network
        self.encoder_scale_network = encoder_scale_network
    ...
    def init(self, environment_spec: specs.EnvironmentSpec):
        ...
        # Create variables for the observation network and get the embedded observation spec.
        emb_spec = tf2_utils.create_variables(self.observation_network, [observation_spec])

        # Define the shape of the generator's output and the Bellman target.
        generator_output_shape = [self.n_gen_samples]

        # Initialize the networks with the appropriate input specs.
        _ = tf2_utils.create_variables(self.generator_network,
                                       [tf.TensorSpec(shape=[self.z_dim], dtype=tf.float32), emb_spec, action_spec])

        _ = tf2_utils.create_variables(self.encoder_loc_network, [tf.TensorSpec(shape=generator_output_shape, dtype=tf.float32),
                                                                emb_spec, action_spec])

        _ = tf2_utils.create_variables(self.encoder_scale_network,[tf.TensorSpec(shape=generator_output_shape,dtype=tf.float32),
                                                                emb_spec, action_spec])

        _ = tf2_utils.create_variables(self.discriminator_network,
                                       [tf.TensorSpec(shape=generator_output_shape, dtype=tf.float32),
                                        tf.TensorSpec(shape=[self.z_dim], dtype=tf.float32),
                                        emb_spec, action_spec ])
...
class GANBuilder:
    ...
    def make_learner(
        self,
        networks: Tuple[D4PGNetworks, D4PGNetworks, GANNetworks, GANNetworks],
        dataset: Iterator[reverb.ReplaySample],
        demo_dataset: Iterator[reverb.ReplaySample] = None,
        counter: Optional[counting.Counter] = None,
        logger: Optional[loggers.Logger] = None,
        ...
        """Creates an instance of the learner."""
        online_networks, target_networks, online_GAN_networks, target_GAN_networks = networks

        # The learner updates the parameters (and initializes them).
        return learning.GANLearner(
            obj_func=self._config.obj_func,
            critic_loss_type=self._config.critic_loss_type,
            threshold=self._config.threshold,
            policy_network=online_networks.policy_network,
            ...
            target_policy_network=target_networks.policy_network,
            target_critic_network=target_networks.critic_network,
            ## GAN's online and arget networks
            gen_network= online_GAN_networks.generator_network,
            disc_network=online_GAN_networks.discriminator_network,
            encoder_loc_network=online_GAN_networks.encoder_loc_network,
            encoder_scale_network=online_GAN_networks.encoder_scale_network,
            target_gen_network = target_GAN_networks.generator_network,
            target_disc_network= target_GAN_networks.discriminator_network,
            target_encoder_loc_network= target_GAN_networks.encoder_loc_network,
            target_encoder_scale_network= target_GAN_networks.encoder_scale_network,
            policy_optimizer=self._config.policy_optimizer,
            critic_optimizer=self._config.critic_optimizer,
            gen_optimizer=self._config.generator_optimizer,
            encoder_optimizer=self._config.encoder_optimizer,
            disc_optimizer=self._config.discriminator_optimizer,
            decay_factor=self._config.decay_factor,  ## decay factor for the intrinsic reward
...
class GAN(agent.Agent):
    """GAN Agent.
    This implements a single-process GAN-D4PG agent. This is an actor-critic algorithm
    that generates data via a behavior policy, inserts N-step transitions into
    a replay buffer, and periodically updates the policy (and as a result the
    behavior) by sampling uniformly from this buffer.
    """
    ...
    def __init__(
        ...
        Args:
          ...
          observation_network: optional network to transform the observations before
            they are fed into any network.
          generator_network: GAN's generator.
          discriminator_network: GAN's discriminator.
          encoder_loc_network: GAN's encoder's location.
          encoder_scale_network: GAN's encoder's scale.
          discount: discount to use for TD updates.
          batch_size: batch size for updates.
          prefetch_size: size to prefetch from replay.
          target_update_period: number of learner steps to perform before updating
        ...
        builder = GANBuilder(
            ...
            GANConfig(
                ...
                policy_optimizer=policy_optimizer,
                critic_optimizer=critic_optimizer,
                generator_optimizer= genrator_optimizer,
        ...
        online_GAN_networks = GANNetworks(observation_network= observation_network,
                                            generator_network=generator_network,
                                            encoder_loc_network=encoder_loc_network,
                                             encoder_scale_network= encoder_scale_network,
                                             discriminator_network= discriminator_network,
                                             z_dim= z_dim,
                                             n_gen_samples= n_gen_samples)

        # Target networks are just a copy of the online networks.
        target_GAN_networks = copy.deepcopy(online_GAN_networks)
        target_networks = copy.deepcopy(online_networks)

        # Initialize the networks.
        online_networks.init(environment_spec)
        target_networks.init(environment_spec)
        online_GAN_networks.init(environment_spec)
        target_GAN_networks.init(environment_spec)

        # TODO(mwhoffman): either make this Dataclass or pass only one struct.
        # The network struct passed to make_learner is just a tuple for the
        # time-being (for backwards compatibility).
        networks = (online_networks, target_networks, online_GAN_networks, target_GAN_networks)
