...
class GANLearner(acme.Learner):
    ...
    def __init__(
        ...
        # Store online and target networks.
        self._policy_network = policy_network
        self._critic_network = critic_network   ## if r_combined is used as an intrinsic reward, besides gen_network, we also need critic_network
    ...
    def _step(self) -> Dict[str, tf.Tensor]:
        ...
        online_variables = (
            ...
            *self._encoder_scale_network.variables,
        )
        target_variables = (
            *self._target_observation_network.variables,
            *self._target_critic_network.variables,
            ...
            *self._target_encoder_scale_network.variables,
        )
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            g_t= self._target_gen_network(z, o_t, self._target_policy_network(o_t)).sample(batch_size) #g_t and sample from it
            
            g_t = tf.reduce_mean(g_t, axis=-1) # not sure if this averaging is okay
            #g_t = tf.reshape(g_t, (-1,1))

            print(g_t.shape) # [batch_size,n_quantiles] without averaging
            #print( tf.reshape(transitions.reward, (-1,1)).shape ) #(bathc_size,1)
            ...
            g_tm1 = self._target_gen_network(z, o_tm1, transitions.action).sample(batch_size)  #### g(s) (and sample from it)
            
            g_tm1 = tf.reduce_mean(g_tm1, axis=-1)
            #g_tm1 = tf.reshape(g_tm1, (-1,1))

            #print(g_tm1.shape) # [batch_size,n_quantile]
            ...
            r_combined= tf.reshape(transitions.reward, (-1,1)) -  decay_rate * tf.reshape(r_intr, (-1,1))   # cause we minimize extrinsic reward and maximize intrinsic reward
            ...
            z = tf.random.normal(shape=(batch_size, self.z_dim))  ##z
            generator_samples = self._target_generator_network(z, o_tm1, transitions.action).values  #### x_tilde
